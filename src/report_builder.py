...
Report Builder
--------------
Shared HTML-rendering logic for the compliance report. Used by BOTH:
  - generate_report.py (writes a static report.html file)
  - app.py (serves the report live, with a "send report" form)

Keeping this in one place means the static file and the live server always
render identically — no duplicated HTML/CSS to keep in sync.
"""
...
# Shared CSS — identical for both the static file and the live server.
BASE_STYLE = """
  :root {
    --paper: #F7F6F2;
    --ink: #20242B;
...
def build_report_html(results, extra_style="", extra_body_top=""):
    """Renders the full report as an HTML string.

    extra_style / extra_body_top let a caller (like the Flask app) inject
    additional CSS and an additional block at the top of the page (e.g. the
    email-send form) without duplicating the whole template.
    """
    ...
    gen_date = (
        ...
        if hasattr(date.today(), "strftime") and __import__("os").name != "nt"
